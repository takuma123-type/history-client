export class NotFoundError extends Error {}
export class UnauthorizedError extends Error {}
export class UnknownError extends Error {}
export class BadRequestError extends Error {}
export class InvlalidSessionTokenError extends Error {}
export class InvalidParameterError extends Error {}
export class FailSignUpError extends Error {}
export class FailLogOutError extends Error {}
export class FailConfirmSignUpCodeError extends Error {}
export class FailResendSignUpCodeError extends Error {}
export class FailCreateJobSeekerError extends Error {}
export class FailGetUserIconPresignedUrlError extends Error{}
export class FailSignInError extends Error{}
export class FailCreateOldGirlError extends Error{}
export class FailUpdateProfileError extends Error{}
export class RepositoryError extends Error {
  constructor(message: string, public readonly details?: any) {
    super(message);
    this.name = "RepositoryError";
  }
}